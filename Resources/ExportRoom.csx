...
// Modified by Pong (for single room)
...
string InputtedText = "r_taverninside3floor";

string roomToExport =  SimpleTextInput("Menu", "Enter the name of Room", InputtedText, true);
...
{
    if (room != null && room.Name.Content == roomToExport) // Добавлена проверка на имя комнаты
    {
        try
        {
            WriteRoomToJson(room);
            break; // Завершаем цикл после нахождения нужной комнаты
        }
        catch (Exception e)
        {
            throw e;
...
void WriteRoomToJson(UndertaleRoom room)
{
    ...
    File.WriteAllBytes(Path.Join(outputPath, room.Name.Content) + ".json", stream.ToArray());
}
